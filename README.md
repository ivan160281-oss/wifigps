...
**On the SD card:**
- `/WIFIGPS` folder at the root
- a new file every 30 minutes of recording: `log_YYYYMMDD_HHMMSS.txt` (name uses local
  UTC+3 time at file-creation time)
- one line every 30 seconds, format: `HH:MM:SS_lat_lon_wifi1,wifi2,wifi3` (nothing after
  the last `_` if no networks are visible)
- if GPS has no valid fix, no line is written, but the 30-second timer and the 30-minute
  file rotation keep running
