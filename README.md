...
1. Create `/WIFIGPS/sync_config.txt` on the SD card (edit it on a computer,
   ...
   1, for backward compatibility with older single-network config files;
   `server_url` should NOT have a trailing slash, and can be `http://` or
   `https://` - https is properly certificate-verified against the real
   ISRG Root X1 (Let's Encrypt) root, not skipped/insecure, so use it
   whenever the server is reachable over HTTPS (any normal reverse-proxy
   setup - Caddy, certbot+nginx, etc. - provides this). Plain `http://` still
   works too, e.g. for a server on your local network with no TLS in front
   of it. `sync_password` must match the server's `WIFIGPS_PASSWORD` - the
   server requires a login for everything now, and since the tracker can't
   do a browser login, it sends this password as a request header instead)
...
- HTTPS sync uses `WiFiClientSecure` with a hardcoded ISRG Root X1 (Let's
  Encrypt root CA, valid to 2035) - see `beginHttpRequest`. Real certificate
  validation on ESP32 also needs the device's clock actually set (mbedTLS
  checks the cert's validity dates against it), which this board otherwise
  never does on its own - `runWifiSync` does a quick NTP sync right after
  connecting, before either HTTPS call, specifically for this
