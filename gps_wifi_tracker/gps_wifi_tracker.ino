/*
 * GPS + WiFi + BLE tracker for LILYGO T-LoRa Pager (ESP32-S3, LilyGoLib)
 * -----------------------------------------------------------------------------
 ...
 *  - "U" (idle only, i.e. not while recording) - WiFi sync: connects to a
 *    WiFi network configured in /WIFIGPS/sync_config.txt on the SD card
 *    (simple ssidN=/passwordN= pairs, plus server_url=/sync_password=; see
 *    below), asks the server which log files it doesn't have yet, and
 ...
 *  - two colored count chips, same colors as the server's map: WiFi
 *    (blue, last scan's network count), BLE (green, last scan's device
 *    count)
 *  - status line: sat count, coordinates, track length (km)
 *
 ...
 * recording, WiFi/BLE scanning all keep running while the screen
 * is off.
 *
 * Buzzer (via the onboard ES8311 codec + speaker) - all beeps are at half
 * the codec's usual volume:
 ...
 * always "NA" - the firmware doesn't calculate speed at all anymore; the
 * field is kept only so the log format/server parser don't need to change.
 * Each WiFi network is logged as ssid|bssid|rssi. The field between the WiFi
 * and BLE lists is always empty now (LoRa/Meshtastic support was removed
 * entirely - no scanning, no listening, nothing recorded - but the field's
 * position is kept so the format doesn't shift). Each BLE sighting is logged
 * as <mac>|rssi (passive scan - no pairing/connecting; many phones/wearables
 ...
 *
 * WiFi sync supports multiple candidate networks (e.g. a home network and a
 * car's mobile hotspot): sync_config.txt lists them as ssid1=/password1=,
 * ssid2=/password2=, etc (a bare ssid=/password= with no number also works,
 * treated as network 1, for backward compatibility with older config
 * files). At sync time, the device tries each configured network in turn
 * and uses whichever one actually connects.
 *
...
// ISRG Root X1 - Let's Encrypt's root CA, valid until 2035. Used to properly
// verify the server's certificate over HTTPS (see httpSyncCheck /
// uploadFileToServer) rather than skipping verification with setInsecure() -
// this is the actual root, not a leaf/intermediate, so it keeps working
// across the server's normal certificate renewals without needing a firmware
// update, as long as the server's chain still terminates here (true for any
// Let's Encrypt-issued certificate, which is what most reverse proxies -
// Caddy, certbot+nginx, etc. - provision by default).
...
// one per line. Supports multiple candidate networks (e.g. home + a car's
// mobile hotspot) - tried in order at sync time, whichever connects first
// is used. A bare ssid=/password= (no number) is also accepted as network 1,
// for backward compatibility. server_url= can be http:// or https:// - https
// is verified against the real ISRG Root X1 (Let's Encrypt) certificate, see
// beginHttpRequest - use it whenever the server is reachable over HTTPS,
// which any normal reverse-proxy setup (Caddy, certbot+nginx, etc.) provides.
...
#define NTP_SYNC_TIMEOUT_MS     8000UL   // see runWifiSync - needed once, before any HTTPS request
...
#define MAX_SYNC_NETWORKS 5
String syncSsids[MAX_SYNC_NETWORKS];
String syncPasswords[MAX_SYNC_NETWORKS];
int syncNetworkCount = 0;
...
// Header widgets: recording indicator (red ball + line count) and two
// colored network-count chips (WiFi/BLE), same colors as the
// server's map.
lv_obj_t *recDot;
lv_obj_t *recCountLabel;
lv_obj_t *wifiChip, *bleChip;
lv_obj_t *wifiChipLabel, *bleChipLabel;
...
static void initAudio() {
    ...
    instance.codec.setVolume(40); // halved from 80 - all beeps are twice as quiet now
...
static void writeLogLine() {
    ...
    // Speed calculation was removed - this field is always "NA" now, kept only
    // so the log format/server parser don't need to change.
    const char *speedStr = "NA";

    // LoRa/Meshtastic support was removed - this field is always empty now,
    // its position kept only so the log format/server parser don't need to change.
    const char *loraStr = "";
...
// ---------------------------------------------------------------------------
// BLE passive scanning (listen-only, same philosophy as WiFi - no
...
static void buildTrackScreen() {
    ...
    // Two colored count chips (WiFi/BLE), same colors as the
    // server's map, so the association is visual rather than needing a label.
    struct { lv_obj_t **chip; lv_obj_t **label; lv_color_t color; int x; } chips[2] = {
...
static void updateTrackScreen() {
    ...
    // Colored network chips - counts only
...
static bool loadSyncConfig() {
    ...
    String ssidsByIdx[MAX_SYNC_NETWORKS + 1];     // index 0 unused, 1..MAX_SYNC_NETWORKS
    String passwordsByIdx[MAX_SYNC_NETWORKS + 1];
    syncServerUrl = ""; syncApiPassword = "";
    ...
    while (f.available()) {
        ...
        if (key == "server_url") { syncServerUrl = val; continue; }
        if (key == "sync_password") { syncApiPassword = val; continue; }

        // Bare "ssid"/"password" (no number) = network 1, for backward
        // compatibility with older single-network config files.
        if (key == "ssid") { ssidsByIdx[1] = val; continue; }
        if (key == "password") { passwordsByIdx[1] = val; continue; }

        // Numbered "ssidN"/"passwordN" - multiple candidate networks (e.g. a
        // home network and a car's mobile hotspot). Tried in order at sync
        // time; whichever one actually connects gets used.
        if (key.startsWith("ssid")) {
            int idx = key.substring(4).toInt();
            if (idx >= 1 && idx <= MAX_SYNC_NETWORKS) ssidsByIdx[idx] = val;
        } else if (key.startsWith("password")) {
            int idx = key.substring(8).toInt();
            if (idx >= 1 && idx <= MAX_SYNC_NETWORKS) passwordsByIdx[idx] = val;
        }
    }
    f.close();
    ...
    syncNetworkCount = 0;
    for (int i = 1; i <= MAX_SYNC_NETWORKS; i++) {
        if (ssidsByIdx[i].length() > 0) {
            syncSsids[syncNetworkCount] = ssidsByIdx[i];
            syncPasswords[syncNetworkCount] = passwordsByIdx[i];
            syncNetworkCount++;
        }
    }

    syncConfigLoaded = (syncNetworkCount > 0 && syncServerUrl.length() > 0);
...
// Starts an HTTPClient request against `url`, using a properly-verified
// HTTPS connection (WiFiClientSecure + the ISRG Root X1 CA above) when the
// URL is https://, or a plain connection otherwise - lets server_url= in
// sync_config.txt be either scheme without duplicating this logic at every
// call site. `secureClient` must outlive the request, so the caller
// declares it in the same scope as `http` and passes it in by reference.
static void beginHttpRequest(HTTPClient &http, WiFiClientSecure &secureClient, const String &url) {
...
static int httpSyncCheck(String *names, int nameCount, String *outMissing, int maxOut) {
    ...
    beginHttpRequest(http, secureClient, syncServerUrl + "/api/sync/check");
...
static int uploadFileToServer(const String &filename) {
    ...
    http.setTimeout(HTTP_TIMEOUT_MS);
    http.setFollowRedirects(HTTPC_FORCE_FOLLOW_REDIRECTS);
    beginHttpRequest(http, secureClient, syncServerUrl + "/api/upload");
...
// needs to run alongside GPS/WiFi-scan in the background.
static void runWifiSync() {
    ...
    for (int i = 0; i < syncNetworkCount && !connected; i++) {
        syncStatus("Connecting to WiFi (" + String(i + 1) + "/" + String(syncNetworkCount) + "):\n"
                   + syncSsids[i] + " ...");
        WiFi.begin(syncSsids[i].c_str(), syncPasswords[i].c_str());
    ...
    if (!connected) {
        syncStatus("Sync failed:\ncould not connect to any of " + String(syncNetworkCount) +
                   " configured network(s)\nwithin " + String(WIFI_CONNECT_TIMEOUT_MS / 1000) +
                   "s each.\n\nPress ENTER to go back.");
        return;
    }

    // HTTPS certificate validation (see beginHttpRequest) checks the
    // server cert's validity dates against this device's own clock - which
    // is never otherwise set (no RTC battery, no GPS-time wiring for this),
    // so without this every HTTPS request would fail even with a perfectly
    // valid certificate. Now that WiFi is up, a quick NTP sync fixes that.
    // Bounded wait so an unreachable/blocked NTP server can't hang the sync
    // forever - if it never syncs, the HTTPS attempt below will fail with a
    // clear certificate-date error rather than this hanging silently.
    syncStatus("Connected.\nSyncing time (needed for HTTPS)...");
    configTime(0, 0, "pool.ntp.org", "time.google.com");
    {
        unsigned long start = millis();
        time_t now = time(nullptr);
        while (now < 1700000000L && millis() - start < NTP_SYNC_TIMEOUT_MS) {  // ~Nov 2023 sanity floor
            delay(200);
            now = time(nullptr);
        }
        Serial.printf("Time after NTP sync attempt: %ld (%s)\n", (long)now, now >= 1700000000L ? "ok" : "FAILED");
    }
...
// charging. GPS/recording/WiFi/BLE scanning all keep running
