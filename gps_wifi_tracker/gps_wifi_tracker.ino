 ...
 *  - One line written every 30 seconds: HH:MM:SS_lat_lon_wifi1,wifi2,wifi3
 *    (time is UTC+3; if no networks are visible, nothing after the last '_')
...
// Writes one line to the current file: HH:MM:SS_lat_lon_wifi1,wifi2,...
static void writeLogLine() {
    if (!currentLogFile) return;
    if (!instance.gps.location.isValid()) return; // no valid coordinates -> skip this line
