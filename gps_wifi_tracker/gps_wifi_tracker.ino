 ...
 *    uploads only those. sync_password must match the server's WIFIGPS_PASSWORD
 *    (sent as a request header, since the device can't do a browser login).
 *    Fully blocking with an on-screen progress/result message; press any key
 *    to dismiss the result and return to normal.
 *
