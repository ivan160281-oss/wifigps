 ...
 *    (simple ssid=/password=/server_url= text file, edited on a computer),
 *    asks the server which log files it doesn't have yet, and uploads only
 *    those. Fully blocking with an on-screen progress/result message; press
 *    any key to dismiss the result and return to normal.
 *
...
static bool loadSyncConfig() {
    ...
    syncSsid = ""; syncPassword = ""; syncServerUrl = "";
    while (f.available()) {
...
static void runWifiSync() {
    ...
    static String allFiles[MAX_SYNC_FILES];
    int fileCount = listSdTxtFiles(allFiles, MAX_SYNC_FILES);
    if (fileCount == 0) {
    ...
    static String missing[MAX_SYNC_FILES];
    int missingCount = httpSyncCheck(allFiles, fileCount, missing, MAX_SYNC_FILES);
    if (missingCount < 0) {
    ...
    for (int i = 0; i < missingCount; i++) {
        ...
        if (code == 200) uploaded++;
        else failed++;
    }

    WiFi.disconnect();
    syncStatus("Sync done: " + String(uploaded) + " uploaded, " + String(failed) + " failed.\n\nPress ENTER to go back.");
}
